...
<form class="layui-form" action="">
    ...
    <div class="layui-form-item">
        <div class="layui-inline">
            ...
            <div class="layui-input-inline">
                <input type="text" name="address"  value="${card.address}"  autocomplete="off" class="layui-input">
            </div>
        </div>
        <div class="layui-inline">
            <label class="layui-form-label">电话</label>
            <div class="layui-input-inline">
                <input type="text" name="mobile"  value="${card.mobile}"  autocomplete="off" class="layui-input">
            </div>
        </div>
    </div>
    <div class="layui-form-item">
        ...
        <div class="layui-input-block">
            <c:forEach var="status" items="${ismrjzks}">
                <c:if test="${status.codeValue == card.relationship}">
                    <input type="radio" name="relationship" value="${status.codeValue}" title="${status.codeValueName}" checked="true">
                </c:if>
                <c:if test="${status.codeValue != card.relationship}">
                    <input type="radio" name="relationship" value="${status.codeValue}" title="${status.codeValueName}"    >
                </c:if>
            </c:forEach>
        </div>
    </div>
        ...
        <div class="layui-input-block">
            <c:forEach var="status" items="${cardstatus}">
                <c:if test="${status.codeValue == card.status}">
                    <input type="radio" name="status" value="${status.codeValue}" title="${status.codeValueName}"   checked="true">
                </c:if>
                <c:if test="${status.codeValue != card.status}">
                    <input type="radio" name="status" value="${status.codeValue}" title="${status.codeValueName}"   >
                </c:if>
            </c:forEach>
        </div>
    </div>
...
<script>
    layui.use(['form'], function(){
        ...
        form.on('submit(submit)', function(data){
            $.post("${pageContext.request.contextPath}/shh/card/cardManager/updateCard",data.field,function(res){
                if(!res.error){
