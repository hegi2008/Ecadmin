...
<div class="container-fluid" style="padding:40px 40px 0px 40px;">
    <form id="bodyCateForm" class="form-horizontal">
        <input id="cate_id" name="cate_id" type="hidden" value="${bodyCate.cate_id }"/>
        <div class="form-group">
            <label for="parent_id" class="col-xs-3 col-sm-3">身体部位父分类</label>
            <div class="col-xs-9 col-sm-9">
                <select class="form-control" id="parent_id" name="parent_id">
                    <option value="0">顶级分类</option>
                    <c:forEach items="${cates }" var="cate">
                        <c:if test="${bodyCate.parent_id == cate.cate_id }">
        ...
        <div class="form-group">
            <label for="cate_name" class="col-xs-3 col-sm-3">身体部位名称</label>
            <div class="col-xs-9 col-sm-9">
                <input id="cate_name" name="cate_name" value="${bodyCate.cate_name }" type="text"
        ...
        <div class="form-group">
            ...
            <div class="col-xs-9 col-sm-9">
                <select class="form-control" id="status" name="status">
                    <c:if test="${bodyCate.status == 1 }">
                        <option value="1" selected="selected">启用</option>
                        <option value="0">注销</option>
                    </c:if>
                    <c:if test="${bodyCate.status == 0 }">
        ...
        <div class="form-group">
            ...
            <div class="col-xs-9 col-sm-9">
                <input id="sort_order" name="sort_order" type="text" value="${bodyCate.sort_order }"
                       maxlength="100" class="form-control" required="true"/>
            </div>
        </div>
        <div class="form-group">
            <label for="cate_type" class="col-xs-3 col-sm-3">身体部位类型</label>
            <div class="col-xs-9 col-sm-9">
                <select class="form-control" id="cate_type" name="cate_type">
                    <c:forEach items="${types }" var="type">
                        <c:if test="${bodyCate.cate_type == type.codeValue }">
...
<script type="text/javascript">
    function saveBodyCate() {
        ...
        var url = Base.globvar.basePath + "shh/body/bodyManager/updateBodyCate";
        $.post(url, $("#bodyCateForm").serializeArray(), function (data) {
            if (!data.error) {
                $("#warning").text("修改身体部位成功").css("display", "block");
                setTimeout(function () {
